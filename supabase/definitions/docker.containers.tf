...
resource "docker_volume" "storage_data" {
  name  = "storage_data"
  count = local.config.supabase.enable_storage ? 1 : 0
  lifecycle {
    prevent_destroy       = false
    create_before_destroy = true
  }
...
resource "docker_container" "supabase-postgres" {
  ...
  name  = local.databases.postgres.postgres_host
  ports {
    internal = local.databases.postgres.postgres_port
    external = local.databases.postgres.postgres_port
  }
  networks_advanced {
    name    = "supabase-network"
    aliases = ["db"]
...
resource "docker_container" "supabase-studio" {
  ...
  env = [
    ...
    "STUDIO_PG_META_URL=http://${local.config.supabase.pg_meta_url}:${local.config.supabase.pg_meta_port}",
  ]
}
...
resource "docker_container" "supabase-kong" {
  ...
  upload {
    content = templatefile(abspath("${path.module}/volumes/api/kong.tpl"), {
      ...
      META_URL   = local.config.supabase.pg_meta_url,
      META_PORT  = local.config.supabase.pg_meta_port,
    })
...
resource "docker_container" "pg-meta" {
  ...
  env = [
    "PG_META_PORT=${local.config.supabase.pg_meta_port}",
    "PG_META_DB_HOST=${local.databases.postgres.postgres_host}",
...
resource "docker_container" "supabase-realtime" {
  ...
  env = [
    "DB_HOST=${local.databases.postgres.postgres_host}",
    "DB_PORT=${local.databases.postgres.postgres_port}",
    "DB_NAME=${local.databases.postgres.postgres_db}",
    "DB_USER=${local.databases.postgres.user}",
...
resource "docker_container" "supabase-storage" {
  ...
  env = [
    ...
    "DATABASE_URL=postgres://${local.databases.postgres.user}:${random_password.POSTGRES_PASSWORD.result}@${local.databases.postgres.postgres_host}:${local.databases.postgres.postgres_port}/postgres",
...
resource "docker_container" "supabase-auth" {
  ...
  env = [
    ...
    "GOTRUE_DB_DATABASE_URL=postgres://${local.databases.postgres.user}:${random_password.POSTGRES_PASSWORD.result}@${local.databases.postgres.postgres_host}:${local.databases.postgres.postgres_port}/${local.databases.postgres.postgres_db}?search_path=auth",
...
resource "docker_container" "supabase-rest" {
  ...
  env = [
    "PGRST_DB_URI=postgres://${local.databases.postgres.user}:${random_password.POSTGRES_PASSWORD.result}@${local.databases.postgres.postgres_host}:${local.databases.postgres.postgres_port}/${local.databases.postgres.postgres_db}",
